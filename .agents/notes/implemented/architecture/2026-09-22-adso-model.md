...
- **op-log 记录单位是「调用者可见意图」**：`ModelOp { kind, at, args, summary }`——kind 是动词、args 是调用者实参（不含 xml；对象实参在记录时做浅快照，账本免疫调用侧事后改写，见 [P3 待办清账](2026-09-22-p3-backlog-sweep.md)）、summary 是含技术名的一句话。不记 diff 片段：v1 不做三方合并/冲突重放（P3+），`plan()` 即 diff 预览（空账本返回 `(no pending ops)`）。
