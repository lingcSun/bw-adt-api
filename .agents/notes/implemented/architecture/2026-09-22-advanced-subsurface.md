...
- **delete 自管锁**：内部域锁原语（`lockADSO`/`lockTransformation`/`lockDTP`）→ `BWObject.delete({lockHandle, transport?})` → 成功即返回、**不再**域级 unlock——真机卡带 lock→DELETE /m 全 200 即成立（2026-09-22 录制，见[卡带闭环](2026-09-22-cassette-recording.md)），删除即释放锁；delete 失败时锁还挂着，做一次 best-effort unlock（吞错，不掩盖原异常）再上抛。options 从 `{lockHandle?, transport?}` 收窄为 `{transport?}`——**0.x minor 行为变化**（lockHandle 不再是调用方输入）。
...
- **delete 走 withWriteSession 引擎**：引擎契约是 lock→steps→unlock-finally（[会话模型](2026-09-18-write-session-model.md)/[引擎](2026-09-21-write-session-engine.md)），与"删除即释放锁、成功不 unlock"直接冲突——要么给引擎开 delete 特例，要么在引擎外重写编排（违反"编排只在 writeSession.ts 维护"不变量）。delete 的就地 try/catch 只有两步且清理语义不同，是最小诚实实现。
...
- delete 成功路径依赖"删除即释放锁"的服务端行为——证据为真机卡带（lock→DELETE 全 200）与 VERIFIED_APIS §8 的端到端记录；失败路径的 best-effort unlock 与 dtp.activate 的 finally 清理同哲学（吞错、不掩盖原异常）。
...
- 删除即释放锁的证据：[真机卡带录制闭环](2026-09-22-cassette-recording.md)；`docs/VERIFIED_APIS.md` §8。
